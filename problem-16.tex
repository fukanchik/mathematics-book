...
\section{Задача}
Каковы должны быть размеры консервной банки цилиндрической формы, чтобы на её
изготовление пошло наименьшее количество материала, если объем банки 0,5 литра?

Один литр равен одному кубическому дециметру или тысяче кубических сантиметров.

\section{Решение}
В задаче нужно минимизировать функцию $S(r,h)$. При этом $r$ и $h$ взаимосвязаны
через указанный фиксированный объём. Поэтому можно выбрать одну переменную,
например $r$, а вторую выразить через неё: $h=f(r)$. Получится, что площадь
зависит только от одной переменной - $r$: минимизируем $S(r)$.
...
Тут придётся взять вторую производную чтобы убедиться что это минимум а не
максимум:
